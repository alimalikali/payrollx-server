...
const transformEmployee = (dbRecord) => {
  ...
  const bonus = parseNumber(dbRecord.bonus);
  const overtimeRate = parseNumber(dbRecord.overtime_rate);
  const providentFundEmployee = parseNumber(dbRecord.provident_fund_employee);
  const providentFundEmployer = parseNumber(dbRecord.provident_fund_employer);
  const fullName = dbRecord.full_name_display || `${firstName} ${lastName}`.trim();
  ...
  return {
    ...
    userRole: dbRecord.user_role || null,
    firstName,
    lastName,
    fullName,
    name: fullName,
    ...
    residentialAddress: dbRecord.residential_address || dbRecord.address || null,
    city: dbRecord.city || null,
    nationality: dbRecord.nationality || null,
    ...
    jobTitle: dbRecord.job_title || dbRecord.designation || null,
    ...
    probationPeriodMonths: dbRecord.probation_period_months ?? null,
    workLocation: dbRecord.work_location || null,
    ...
    bankRoutingCode: dbRecord.bank_routing_code || null,
    bankBranch: dbRecord.bank_branch || null,
    ntnNumber: dbRecord.ntn_number || null,
    taxIdentifier: dbRecord.tax_identifier || dbRecord.ntn_number || null,
    taxInformation: dbRecord.tax_information || null,
    legalIdType: dbRecord.legal_id_type || null,
    legalIdNumber: dbRecord.legal_id_number || null,
    taxFilingStatus: dbRecord.tax_filing_status || 'non_filer',
    paymentMethod: dbRecord.payment_method || 'bank_transfer',
    ...
    jobDetails: {
      ...
      probationPeriodMonths: dbRecord.probation_period_months ?? null,
      workLocation: dbRecord.work_location || null,
      reportingManagerId: dbRecord.reporting_to || dbRecord.manager_id || null,
      reportingManagerName: dbRecord.manager_first_name
        ? `${dbRecord.manager_first_name} ${dbRecord.manager_last_name || ''}`.trim()
        : null,
    },
    salaryDetails: {
      ...
      allowances: {
        hra: parseNumber(dbRecord.housing_allowance || dbRecord.hra),
        travel: parseNumber(dbRecord.transport_allowance),
        medical: parseNumber(dbRecord.medical_allowance),
        utility: parseNumber(dbRecord.utility_allowance),
        other: parseNumber(dbRecord.other_allowances),
      },
      bonus,
      overtimeRate,
      taxInformation: dbRecord.tax_information || null,
